...
"""
Code taken from https://github.com/DataResponsibly/FairRank/blob/master/dataGenerator.py
"""
...
def completeCheckRankingProperties(_ranking,_protected_group):    
    ...
    if not isinstance(_ranking, (list, tuple, np.ndarray)) and not isinstance( _ranking ):
        raise TypeError("Input ranking must be a list-wise structure defined by '[]' symbol")
    if not isinstance(_protected_group, (list, tuple, np.ndarray)) and not isinstance( _protected_group ):
